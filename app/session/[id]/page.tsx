...
import { MobileConnectionManager } from "@/lib/mobile-connection-manager"
...
export default function SessionPage() {
  ...
  const [connectionStrength, setConnectionStrength] = useState<"excellent" | "good" | "poor">("good")
  const [reconnectAttempts, setReconnectAttempts] = useState(0)
  ...
  // Core system refs
  const fileInputRef = useRef<HTMLInputElement>(null)
  const bulletproofP2PRef = useRef<BulletproofP2P | null>(null)
  const mobileManagerRef = useRef<MobileConnectionManager | null>(null)
  const sessionTimerRef = useRef<NodeJS.Timeout | null>(null)

  // Initialize bulletproof P2P system with mobile support
  useEffect(() => {
    if (!user || !sessionId) return

    console.log("🚀 Initializing Bulletproof P2P System with Mobile Support")

    // Initialize mobile connection manager first
    const mobileManager = new MobileConnectionManager()
    mobileManagerRef.current = mobileManager

    // Initialize bulletproof P2P
    const bulletproofP2P = new BulletproofP2P(sessionId, user.id, mobileManager)
    bulletproofP2PRef.current = bulletproofP2P

    // Mobile-specific event handlers
    mobileManager.onBackgroundStateChange = (isInBackground) => {
      setIsBackground(isInBackground)
      console.log(`📱 App ${isInBackground ? "backgrounded" : "foregrounded"}`)
    }

    mobileManager.onNetworkChange = (type) => {
      setNetworkType(type)
      console.log(`📶 Network changed to: ${type}`)
    }

    mobileManager.onConnectionStrengthChange = (strength) => {
      setConnectionStrength(strength)
    }

    // Core connection event handlers
    bulletproofP2P.onConnectionStatusChange = (status) => {
      setConnectionStatus(status)
    }

    bulletproofP2P.onSignalingStatusChange = (status) => {
      setWsStatus(status)
    }

    bulletproofP2P.onUserCountChange = (count) => {
      setUserCount(count)
    }

    bulletproofP2P.onError = (errorMsg) => {
      setError(errorMsg)
    }

    bulletproofP2P.onConnectionQualityChange = (quality) => {
      setConnectionQuality(quality)
    }

    bulletproofP2P.onSpeedUpdate = (speed) => {
      setCurrentSpeed(speed)
    }

    bulletproofP2P.onReconnectAttempt = (attempts) => {
      setReconnectAttempts(attempts)
    }

    // File transfer event handlers
    bulletproofP2P.onFileTransferUpdate = (transfers) => {
      setFileTransfers(transfers)
    }

    bulletproofP2P.onChatMessage = (message) => {
      setChatMessages((prev) => [...prev, message])
    }

    // Initialize systems
    mobileManager.initialize()
    bulletproofP2P.initialize()
  ...
  const handleVisibilityChange = () => {
    if (document.hidden) {
      console.log("📱 Page hidden - maintaining connection in background")
      bulletproofP2PRef.current?.handleBackgroundMode(true)
    } else {
      console.log("📱 Page visible - resuming foreground mode")
      bulletproofP2PRef.current?.handleBackgroundMode(false)
    }
  }

  const handlePageHide = () => {
    console.log("📱 Page hide event - preserving connection")
    bulletproofP2PRef.current?.preserveConnection()
  }

  const handlePageShow = () => {
    console.log("📱 Page show event - restoring connection")
    bulletproofP2PRef.current?.restoreConnection()
  }
  ...
  const handlePreviewSend = async (files: File[]) => {
    if (bulletproofP2PRef.current) {
      await bulletproofP2PRef.current.sendFiles(files)
    }
    setPreviewFiles([])
    setShowPreview(false)
  }
  ...
  const handleSendChatMessage = (content: string, type: "text" | "clipboard") => {
    if (bulletproofP2PRef.current) {
      bulletproofP2PRef.current.sendChatMessage(content, type, user?.firstName || "You")
    }
  }
  ...
  const handleReconnect = () => {
    setError("")
    if (bulletproofP2PRef.current) {
      bulletproofP2PRef.current.forceReconnect()
    }
  }
  ...
  const getNetworkIcon = () => {
    ...
    if (networkType.includes("cellular")) return "📱"
    if (networkType.includes("ethernet")) return "🌐"
    return "📡"
  }
    ...
    <div className="min-h-screen bg-purple-300 p-2 md:p-4">
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-4 md:mb-6">
          ...
          <div className="flex items-center justify-center gap-2 md:gap-4 flex-wrap">
            ...
            {isMobile && (
              <>
                <div
                  className={`flex items-center gap-1 px-2 py-1 border-2 border-black font-black text-xs ${
                    isBackground ? "bg-orange-400" : "bg-green-400"
                  }`}
                >
                  {getNetworkIcon()} {networkType.toUpperCase()}
                </div>

                {reconnectAttempts > 0 && (
                  <div className="flex items-center gap-1 px-2 py-1 border-2 border-black font-black bg-yellow-400 text-xs">
        ...
        {isMobile && isBackground && (
          <Card className="neubrutalism-card bg-orange-300 mb-4 md:mb-6">
            <CardContent className="p-3 md:p-4 flex items-center gap-2">
              <Smartphone className="w-4 md:w-5 h-4 md:h-5 flex-shrink-0" />
              <span className="font-bold flex-1 text-sm md:text-base">
                App is in background - Connection maintained via Service Worker
              </span>
            </CardContent>
          </Card>
        )}
            ...
            <Card className="neubrutalism-card bg-yellow-300">
              ...
              <CardContent>
                ...
                >
                  ...
                  <p className="text-xs md:text-sm font-bold mt-4 text-gray-600">
                    Max 1GB per file • Multi-file support • AI Scanned • SHA-256 verified • Bulletproof transfer
                  </p>
                </div>
              </CardContent>
            </Card>
                  ...
                  <div className="text-center space-y-4">
                    {wsStatus === "connecting" && (
                      <div className="bg-yellow-200 p-4 md:p-6 border-4 border-black">
                        ...
                        {isMobile && <p className="text-xs mt-2">Mobile optimized • Background support</p>}
                      </div>
                    )}
                    ...
                    {wsStatus === "connected" && userCount < 2 && (
                      <div className="bg-yellow-200 p-4 md:p-6 border-4 border-black">
                        ...
                        <p className="font-bold text-sm md:text-base">Share the session code with your friend!</p>
                        <p className="text-xs md:text-sm mt-2">Users in session: {userCount}/2</p>
                        {isMobile && <p className="text-xs mt-2">Keep app open for instant connection</p>}
                      </div>
                    )}
                    ...
                    {wsStatus === "connected" && userCount === 2 && connectionStatus === "connecting" && (
                      <div className="bg-orange-200 p-4 md:p-6 border-4 border-black">
                        ...
                        {reconnectAttempts > 0 && <p className="text-xs mt-2">Retry attempt: {reconnectAttempts}</p>}
                        <Button
                          onClick={handleReconnect}
                          className="neubrutalism-button bg-orange-500 text-white mt-4 touch-target"
                          size="sm"
                      ...
                      <div className="bg-green-200 p-4 md:p-6 border-4 border-black">
                        ...
                        {isMobile && (
                          <p className="text-xs mt-2">
                            Network: {networkType} • Background: {isBackground ? "Active" : "Inactive"}
                          </p>
                        )}
                      </div>
                    )}
                    ...
                    {(wsStatus === "disconnected" || wsStatus === "error") && (
                      <div className="bg-red-200 p-4 md:p-6 border-4 border-black">
                        ...
                        <p className="font-black text-base md:text-lg text-red-800">CONNECTION FAILED</p>
                        <p className="font-bold mb-4 text-sm md:text-base">Auto-reconnecting...</p>
                        {reconnectAttempts > 0 && <p className="text-xs mb-2">Attempts: {reconnectAttempts}</p>}
                        <Button
                          onClick={handleReconnect}
                          className="neubrutalism-button bg-red-500 text-white touch-target"
                        >
