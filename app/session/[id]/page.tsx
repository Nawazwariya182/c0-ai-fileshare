...
import { useUser } from "@clerk/nextjs"
import { useParams, useRouter } from "next/navigation"
import { Users, Wifi, WifiOff, AlertTriangle, RefreshCw, Shield, Smartphone, Monitor } from "lucide-react"
...
export default function SessionPage() {
  ...
  const fileChunksRef = useRef<Map<string, { chunks: ArrayBuffer[]; totalChunks: number; receivedChunks: number }>>(
    new Map(),
  )

  // WebRTC Configuration with multiple STUN/TURN servers for reliability
  ...
  const initializeWebSocket = useCallback(() => {
    ...
    const wsUrl =
      process.env.NODE_ENV === "production" ? "wss://your-signaling-server.herokuapp.com" : "ws://localhost:8080"
  ...
  const handleSignalingMessage = useCallback(async (message: any) => {
    ...
    switch (message.type) {
      ...
      case "joined":
        ...
        if (message.userCount === 2 && message.readyForConnection) {
          // Both users are present, start P2P connection
          setTimeout(() => {
      ...
      case "user-joined":
        ...
        if (message.readyForConnection && isInitiatorRef.current) {
          setTimeout(() => {
            initiatePeerConnection()
          }, 500)
        }
  ...
  const initiatePeerConnection = useCallback(async () => {
    ...
    console.log("🚀 Initiating peer connection as", isInitiatorRef.current ? "INITIATOR" : "RECEIVER")
    ...
    try {
      ...
      pc.onconnectionstatechange = () => {
        ...
        peerConnection.connectionState = pc.connectionState
        ...
        switch (pc.connectionState) {
          ...
          case "failed":
            ...
            console.log("❌ P2P connection failed, attempting reconnection...")
      ...
      // Create data channel for file transfers (initiator only)
      if (isInitiatorRef.current) {
        const dataChannel = pc.createDataChannel("fileTransfer", {
          ordered: true,
          maxRetransmits: 3,
        ...
        // Handle incoming data channel (receiver)
        pc.ondatachannel = (event) => {
  ...
  const handleOffer = useCallback(
    ...
    [sessionId, initiatePeerConnection],
  )
  ...
  const handleICECandidate = useCallback(async (message: any) => {
    ...
    const pc = peerConnectionRef.current?.pc
    if (!pc) {
      console.log("⏳ No peer connection yet, storing ICE candidate")
      pendingICECandidatesRef.current.push(new RTCIceCandidate(message.candidate))
      return
    }
  ...
  const handleFileInfo = useCallback((message: any) => {
    console.log("📄 Received file info:", message.fileName)
    ...
    fileChunksRef.current.set(message.fileId, {
      chunks: [],
  ...
  // Handle file chunk
  const handleFileChunk = useCallback((message: any) => {
    const fileData = fileChunksRef.current.get(message.fileId)
    if (!fileData) return
    ...
    fileData.chunks[message.chunkIndex] = bytes.buffer
    ...
    setFileTransfers((prev) =>
      prev.map((transfer) =>
        transfer.id === message.fileId ? { ...transfer, progress, status: "transferring" } : transfer,
      ),
    )

    // Calculate speed
    const now = Date.now()
    const speed = bytes.length / ((now - (message.timestamp || now)) / 1000)
    setCurrentSpeed(speed)

    console.log(`📦 Received chunk ${message.chunkIndex + 1}/${fileData.totalChunks} for ${message.fileId}`)
  }, [])

  // Handle file complete
  const handleFileComplete = useCallback((message: any) => {
    console.log("✅ File transfer complete:", message.fileId)

    const fileData = fileChunksRef.current.get(message.fileId)
    if (!fileData) return

    // Combine all chunks
    const totalSize = fileData.chunks.reduce((sum, chunk) => sum + chunk.byteLength, 0)
    ...
    setFileTransfers((prev) =>
      prev.map((transfer) =>
        transfer.id === message.fileId ? { ...transfer, progress: 100, status: "completed" } : transfer,
      ),
    )

    fileChunksRef.current.delete(message.fileId)
    setCurrentSpeed(0)
  }, [])

  // Send files through data channel
  const sendFiles = useCallback(async (files: File[]) => {
    const dataChannel = peerConnectionRef.current?.dataChannel
    if (!dataChannel || dataChannel.readyState !== "open") {
      setError("No active connection for file transfer")
      return
    }
        ...
        reader.onload = (e) => {
          if (e.target?.result instanceof ArrayBuffer) {
            ...
            if (chunkIndex < totalChunks) {
              ...
              setFileTransfers((prev) => prev.map((t) => (t.id === fileId ? { ...t, status: "completed" } : t)))
  ...
  const handleSendChatMessage = useCallback(
    (content: string, type: "text" | "clipboard") => {
      const dataChannel = peerConnectionRef.current?.dataChannel

      if (dataChannel && dataChannel.readyState === "open") {
        // Send via data channel (P2P)
        dataChannel.send(
          JSON.stringify({
            type: "chat",
            content,
  ...
  }, [user, sessionId, initializeWebSocket])
