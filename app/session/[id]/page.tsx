...
const DEFAULT_DJANGO_URL = (process.env.NEXT_PUBLIC_DJANGO_URL ?? '').replace(/\/$/, '')
...
export default function SessionPage() {
  ...
  useEffect(() => {
    try {
      ...
      if (djangoParam) {
        const trimmed = djangoParam.trim().replace(/\/$/, '')
        localStorage.setItem('DJANGO_BASE_URL', trimmed)
      ...
      if (stored && stored.trim()) {
        setEncryptionUrl(stored)
        return
      }
      
      if (DEFAULT_DJANGO_URL) {
        localStorage.setItem('DJANGO_BASE_URL', DEFAULT_DJANGO_URL)
        setEncryptionUrl(DEFAULT_DJANGO_URL)
      }
    } catch (error) {
      console.warn('Failed to setup encryption URL:', error)
    }
  }, [])

  const handleSetEncryptionServer = () => {
    const current = localStorage.getItem('DJANGO_BASE_URL') || DEFAULT_DJANGO_URL || ''
    const next = window.prompt('Enter Django Encryption Server URL (https://...)', current)
    ...
    try {
      ...
      console.error('Failed to save DJANGO_BASE_URL:', error)
    }
  }
  ...
  const handlePreviewSend = async (files: File[]) => {
    if (p2pRef.current) {
      try {
        ...
        console.log(`📤 Initiated transfer for ${files.length} files`)
      } catch (error) {
    ...
    return (
      <div className="min-h-screen bg-purple-300 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="font-black text-xl">LOADING SESSION...</p>
        </div>
      </div>
    )
  }
      ...
      <div className="max-w-7xl mx-auto">
        <header className="text-center mb-4 md:mb-6">
          ...
          <div className="flex items-center justify-center gap-2 md:gap-4 flex-wrap">
            ...
            <button
              type="button"
              onClick={handleSetEncryptionServer}
              className={`flex items-center gap-1 md:gap-2 px-2 md:px-4 py-1 md:py-2 border-2 md:border-4 border-black font-black text-xs md:text-sm ${
                encryptionUrl ? 'bg-green-400' : 'bg-gray-300'
              }`}
            ...
            >
              ...
              <span>{encryptionUrl ? 'SET' : 'OFF'}</span>
            </button>
          </div>
        </header>
            ...
            <Card className="neubrutalism-card bg-yellow-300">
              ...
              <CardContent>
                <div
                  ...
                  onClick={() => {
                    if (isMobile && connectionStatus === 'connected') fileInputRef.current?.click()
                  }}
                >
                  ...
                  {!isMobile && connectionStatus === 'connected' && <p className="font-bold mb-4">or</p>}
                  ...
                  <div className="space-y-3 md:space-y-4">
                    {fileTransfers.map((transfer) => (
                      <div key={transfer.id} className="bg-white p-3 md:p-4 border-2 border-black">
                        ...
                        <div className="flex justify-between text-xs md:text-sm font-bold mt-1">
                          <span>{transfer.progress}%</span>
                          {transfer.speed && transfer.speed > 0 && (
                            <span>{transfer.speed < 1024 ? `${transfer.speed} B/s` : transfer.speed < 1024 * 1024 ? `${(transfer.speed / 1024).toFixed(1)} KB/s` : `${(transfer.speed / 1024 / 1024).toFixed(1)} MB/s`}</span>
                          )}
                        </div>
                      </div>
                    ))}
